...
| Role | Prefs | Live config |
|------|-------|-------------|
| Root (sudo) | `/root/.local/share/app.hiddify.com/shared_preferences.json` | `/root/.local/share/app.hiddify.com/data/current-config.json` |
| User | `~/.local/share/app.hiddify.com/...` | `~/.local/share/hiddify/...` (legacy installs) |

When Hiddify runs as **root**, user-home config copies are **stale** — always patch/read the **root** `app.hiddify.com` paths for the live daemon.
